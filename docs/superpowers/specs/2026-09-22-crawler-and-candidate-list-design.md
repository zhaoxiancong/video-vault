...
| ~~"临时提速"开关~~ | **本轮不实现** | 爬取流程里根本没有"延时"这个机制，没有可提速的东西（实测单次 1.1 秒）。等真的遇到需要节流的站再单独讨论 |

> 关于那句"默认保守、留个临时提速开关"：**已经和用户确认过本轮不做**。
> 保留在本节里是为了记录这个决定，而不是留一个待办 ——
> 一个点不动的开关比没有开关更容易让人以为"护栏是可关的"。
